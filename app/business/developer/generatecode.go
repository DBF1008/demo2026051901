...
func (api *Generatecode) Save(c *gin.Context) {
	...
	if isok {
		...
		filename_arr := strings.Split(codedata["api_filename"].(string), `.`)
		...
		vue_path := filepath.Join(global.App.Config.App.Vueobjroot, "/src/views/", componentpah_arr[0])
		...
		if codedata["tpl_type"] == "contentcatelist" {
		...
		UpFieldData(filepath.Join(vue_path, "data.ts"), listfield)
		...
		UpFieldAddForm(filepath.Join(vue_path, "AddForm.vue"), codedata["fields"], formfield)
...
func (api *Generatecode) Del(c *gin.Context) {
	...
	var parameter map[string]interface{}
	_ = json.Unmarshal(body, &parameter)
	if parameter["is_install"] != nil && gf.InterfaceToInt(parameter["is_install"]) == 1 {
