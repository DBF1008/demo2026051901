...
func (api *Bizrole) Get_list(c *gin.Context) {
	...
	role_ids := gf.GetAllChilIds("business_auth_role", role_id.([]interface{}))
...
func (api *Bizrole) Get_menuList(c *gin.Context) {
	...
	if id == "0" || pid == "0" {
		...
		pid, _ := model.DB().Table("business_auth_role").Where("id", id).Value("pid")
...
func (api *Bizrole) Save(c *gin.Context) {
	...
	if parameter["menu"] != nil && parameter["menu"] != "*" {
		rules := gf.GetRulesID("business_auth_role", "pid", parameter["menu"])
